<script>
  ...
  export let allKabupaten;
  import { getLocality } from "./LocationService.js";
  ...
  function setKabupatenFilter(kabupaten) {
    filterStore.update((filters) => {
      return { ...filters, kabupaten };
    });
    handleFilterChange();
  }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          location = locality || {};
          setKabupatenFilter(location.location);
        } catch (e) {
...
<div class=" flex gap-2 mx-auto pt-4">
  {#if location}
    ...
    >
      <div class="i-fluent-emoji-person h-5 w-5 mr-3" />
  ...
  {:else}
    ...
    >
      {#if isFetching}
        <svg
          aria-hidden="true"
          class="h-5 w-5 mr-3 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
          viewBox="0 0 100 101"
          fill="none"
        ...
        Loading...
      {:else}
        <div class="i-fluent-emoji-pushpin h-5 w-5 mr-3" />
        Get My Location
      {/if}
    </button>
  {/if}
