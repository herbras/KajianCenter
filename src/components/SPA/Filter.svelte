<script>
    ...
    // Fungsi utilitas tanggal menggunakan date-fns
    function formatDate(dateString) {
        return format(parseISO(dateString), "EEEE, dd MMMM yyyy", { locale: id }).replace('Minggu', 'Ahad');
    }
    ...
    function formatDateTime(dateString) {
        return format(parseISO(dateString), "yyyy-MM-dd'T'HH:mm:ss");
    }
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    {#each $kajianStore as kajian (kajian.id)}
        <article
            class="relative overflow-hidden rounded-lg shadow transition hover:shadow-lg"
        >
            <img
                src={kajian.gambarKajian || "https://pbs.twimg.com/media/GBRTGrpaAAAgmD2?format=jpg&name=large"}
                ...
                <div class="p-4 sm:p-6">
                    <label class="flex-inline">
                        ...
                        {#if kajian.kategoriIlmu}
                            {#each kajian.kategoriIlmu as kategori (kategori)}
                                <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-green-600 bg-green-200 last:mr-0 mr-1">
                                    {kategori}
                                </span>
                            {/each}
                        {/if}
